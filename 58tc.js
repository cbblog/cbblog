...
class UserInfo {
    constructor(str) {
        ...
        this.canCompoundHouse = 200 // 可合成最大次数
    ...
    async doCompoundHouse () {
        ...
        if (!this.maininfo?.levelInfo?.house) {
            console.log(`账号[${this.index}]我的房子没有初始化，退出`);
            return;
        }
        await $.wait(200);
        if (this.compoundHouse < this.canCompoundHouse) {
            ...
            if (!info.flag) {
                ...
                this.sellBuildNum = 0;
                this.compoundHouse++;
                await this.doCompoundHouse(type);
            }
        } else {
            console.log(`账号[${this.index}]本次合成任务已完成，结束`);
        }
    }
    // 梦想小镇-合成建筑
    async compound(info) {
        ...
        if(result.code == 0) {
            ...
            if (taskId) {
                console.log(`账号[${this.index}]正在做任务...`);
                await $.wait(25000);
                // 做任务
                await this.doTask(sceneId, taskId);
                await $.wait(200);
    ...
    async buyBuild() {
        ...
        const minLevel = Math.floor(this.maininfo.levelInfo.house/2)-2 ? Math.floor(this.maininfo.levelInfo.house/2)-1 : 1;
        if (level < minLevel) {
            console.log(`账号[${this.index}]当前能购买的建筑等级过低，结束`);
            return;
        }
